...
class TopKSAE(SparseAutoencoder):
    ...
    def compute_finetune(self, x):
        def mask2low(temp, mask):
            temp = temp.to(mask.device)
            return temp - (1. - mask) * 1e9
        x = x.to(self.b_dec.device)
        h = self._encode(x)
        actvs_1 = h * MaskTopK(mask2low(h, self.norm_mask), self.topk)
        yhat_1 = self._decode(actvs_1)
        actvs_2 = h * MaskTopK(mask2low(h, self.dead_mask), self.topk)
        yhat_2 = self._decode(actvs_2)
        actvs = tc.relu(actvs_1 + actvs_2)
        self.l2 = normalized_l2(self._decode(actvs), x)
        self.l1 = (actvs.abs().sum(dim=1) / x.norm(dim=1)).mean()
        self.l0 = actvs.norm(dim=-1, p=0).mean()
        self.ttl = normalized_l2(yhat_1, x) +\
                   0.1 * normalized_l2(yhat_2, x - yhat_1.detach())
        return self.ttl, self.l2, self.l1, self.l0
