...
def test_CLF(dataset, model):
    ...
    for X1, X2 in dataset:
        ...
        reals.extend(real)
    model.train()
    return accuracy_score(reals, preds), f1_score(reals, preds, average="binary")#np.mean(accs)
...
def train_CLF(train_loader, valid_loader, test1_loader, test2_loader, model, config, loading_only=False):
    ...
    print("\nBest Epoch=%d | Step=%d | ValidAcc=%.4f | ValidACC=%.4f | ValidF1=%.4f | TestACC=%.4f | TestF1=%.4f" % (
...
def load_datasets(seed, layer, model="mistral"):
    train_data = GroupPairActvDataset("datasets/TD_%s_train" % model, layer)
    valid_data = GroupPairActvDataset("datasets/TD_%s_train" % model, layer)
    ...
    test2_data = GroupPairActvDataset("datasets/TD_%s_test" % model, layer)
    test1_data = GroupPairActvDataset("datasets/TD_%s_valid" % model, layer)
...
def choose_untask_mask(fpath, key="Task", task_labels={"yes", "probably"}):
    ...
    return mask  # 1: features has no relation to the task (unintended)
