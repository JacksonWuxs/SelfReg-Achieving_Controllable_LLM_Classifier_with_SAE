...
class GroupPairActvDataset:
    def __init__(self, fpath, layerID, batch_size=1, workers=1):
    ...
    def __len__(self):
        return self._blocksize * (len(self._data) - 1) + len(self._loading_file(self._data[-1]))
...
def extract_layer_last_numpy(fpath, layerID):
    ...
    def loading_file(x):
        data = tc.load(fpath + '/' + x, weights_only=True)
...
if __name__ == "__main__":
    ...
    for l in [int(sys.argv[1])]:
        ...
        extract_layer_last_numpy("datasets/toxichat_mistral_test", l)
        extract_layer_last_numpy("datasets/safety_mistral_train", l)
        #extract_layer_last_numpy("datasets/safety_mistral_test", l)
        extract_layer_last_numpy("datasets/safety_mistral_valid", l)
