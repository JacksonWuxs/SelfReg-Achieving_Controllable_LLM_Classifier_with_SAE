...
if __name__ == "__main__":
    subset = sys.argv[4]
    corpus = CorpusSearchIndex("datasets/RM_%s.tsv" % subset, cache_freq=10000, sampling=None)
    ...
    with tc.no_grad():
        ...
        for i, text in enumerate(corpus):
            chosen, reject = text.split("\t")
            chosen = formatter(chosen.replace("\\n", "\n").replace("\\t", "\t"))
            reject = formatter(reject.replace("\\n", "\n").replace("\\t", "\t"))
            bar.update(1)
            try:
                if i < group_idx * group_size:
                    continue
                fpath = "datasets/RM_mistral_%s/group_%d.pt" % (subset, group_idx)
                if os.path.exists(fpath):
                    group_idx += jump
                    continue
                actvs_chosen = collect_actvs(chosen, model, collectors)
                actvs_reject = collect_actvs(reject, model, collectors)
                group_actvs.append([actvs_chosen, actvs_reject])
